...
async fn exec_er(task: &Task, ctx: Arc<Context>) -> Result<TaskOutput, failure::Error> {
    match task {
        Task::Once(async_task) => async_task.exec(ctx).await,
        Task::Chain(async_tasks) => {
            let mut outputs: Vec<TaskOutput> = vec![];
            for i in async_tasks {
                let output = i.exec(ctx.clone()).await?;
                outputs.push(output);
            }
